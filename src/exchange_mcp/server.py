...
from .ews.contacts import create_contact, delete_contact, get_contact, search_contacts, update_contact
from .ews.email import (
    copy_email,
    create_draft,
    create_folder,
    delete_email,
...
def _create_tool_handler(name: str, registry: ToolRegistry, model: type[BaseModel] | None = None) -> Callable[..., str]:
    import inspect

    if model is None:
        def handler_no_args() -> str:
            payload, is_error = registry.call(name, {})
            if is_error:
    ...
    def _make_closure(registry_ref, tool_name):
        def inner(**kwargs: Any) -> str:
            payload, is_error = registry_ref.call(tool_name, kwargs)
            if is_error:
                raise RuntimeError(json.dumps(payload, ensure_ascii=False))
            return json.dumps(payload, ensure_ascii=False)
        return inner

    handler = _make_closure(registry, name)
    handler.__name__ = f"{name}_handler"

    sig = inspect.Signature(parameters=param_list, return_annotation=str)
...
def build_mcp_server(settings: Settings | None = None, client: ExchangeClient | None = None) -> Any:
    ...
    for name, description, model in tool_configs:
        handler = _create_tool_handler(name, registry, model)

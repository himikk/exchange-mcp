...
class ListEmailsRequest(ExchangeModel):
    folder: str = Field(default="inbox", description="Folder name or path (e.g., 'inbox', 'Входящие', 'inbox/jira', 'Входящие/Projects')")
...
class ListFoldersRequest(ExchangeModel):
    parent: str | None = Field(default=None, description="Parent folder to list children from (e.g., 'inbox', 'Входящие', or full path)")
    depth: int = Field(default=2, ge=0, le=10, description="How many levels of nested folders to include (0=only parent, 10=all nested folders)")
